...
namespace Libriary_BAL.Services
{
        ...
        ) : IAuthorService
    {
        ...
        public async Task<AuthorDTO> CreateAuthorAsync(AuthorDTO authorDTO)
        {
            ...
            var createdAuthor = await _authorRepository.AddAsync(_mapper.Map<Author>(authorDTO));

            return _mapper.Map<AuthorDTO>(createdAuthor);
        }

        public async Task<AuthorDTO> UpdateAuthorAsync(AuthorToUpdateDTO authorToUpdateDTO)
        {
            var author = await _authorRepository.GetAsync(x => x.AuthorId == authorToUpdateDTO.AuthorId);
        ...
        {
            var authorToDelete = await _authorRepository.GetAsync(x => x.AuthorId == id);
